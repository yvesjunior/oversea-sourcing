...
| --- | --- | --- |
| `users` | email (uq), password_hash, first_name, last_name, locale `fr\|en`, `platform_role` `user\|ops\|superadmin`, email_verified_at | `platform_role` gates the admin backoffice (OSI staff) |
...
### E0 — Dev foundations
- [ ] Enable `database` (postgres:16) in both compose files: volume, healthcheck, `depends_on`
- [ ] `DATABASE_URL` wiring — secrets in `.env.local`, never in committed `.env`
- [ ] Drizzle + drizzle-kit: schema layout in `src/database/`, migration workflow, `npm run db:migrate` / `db:seed`
...
- [ ] better-auth setup (email/password, argon2, httpOnly session cookie)
...
*Engager*, OSI ops sees it in the queue, buyer sees "connected", downloads the PDF report.
